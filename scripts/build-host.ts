...
/**
 * Parse CLI arguments. A flag that expects a value must be followed by one —
 * a valueless `--target-triple` used to silently fall back to the host triple.
 */
export function parseArgs(args: string[]): BuildHostArgs {
  const parsed: BuildHostArgs = {}
  for (let i = 0; i < args.length; i++) {
    const arg = args[i]
    if (arg === "--target-triple" || arg === "--triple") {
      const value = args[++i]
      if (value === undefined || value.startsWith("--")) {
        throw new Error(`${arg} requires a Rust target triple (e.g. x86_64-pc-windows-msvc)`)
      }
      parsed.targetTriple = value
    } else if (arg === "--out-dir") {
      const value = args[++i]
      if (value === undefined || value.startsWith("--")) {
...
/**
 * Failure message for a `bun build --compile` that did not produce an artifact.
 * A spawn failure has `status === null` and no stderr — reporting only
 * `stderr ?? ""` printed "undefined"; the real reason is in `error`.
 */
export function compileFailureMessage(input: {
  triple: string
  bunTarget: string
  status: number | null
  ...
  const reason = input.error
    ? `spawn error: ${input.error.message}`
    : input.signal
      ? `killed by signal ${input.signal}`
      : `exit status ${String(input.status)}`
  const details = (input.stderr ?? "").trim()
  return [
    `bun build --compile failed (${reason}; triple=${input.triple}, target=${input.bunTarget})`,
    details,
  ]
    .filter(Boolean)
    .join("\n")
