...
impl<R: Runtime> TrayRenderer<R> {
    ...
    /// Label + action for a menu id. The label is needed for the confirmation
    /// prompt of `confirm`/`danger` actions; `None` for separators/unknown ids.
    pub fn action_entry(&self, id: &str) -> Option<(String, crate::tray_model::TrayAction)> {
        self.rendered
            .as_ref()
            .and_then(|rendered| find_action_entry(&rendered.snapshot, id))
...
fn build_menu<R: Runtime>(
    ...
    for (group_index, group) in snapshot.groups.iter().enumerate() {
        ...
        // A labelled group renders as a native submenu (its label is the
        // section header); an unlabelled group is appended flat.
        match group.label.as_deref().filter(|label| !label.is_empty()) {
            Some(label) => {
                let submenu = Submenu::with_id(app, &group.id, label, true)
                    .map_err(|error| error.to_string())?;
                append_items_to_submenu(app, &submenu, &group.items, &mut handles)?;
                menu.append(&submenu).map_err(|error| error.to_string())?;
            }
            None => append_items_to_menu(app, &menu, &group.items, &mut handles)?,
        }
    }
    Ok(RenderedMenu {
        menu,
...
fn update_items<R: Runtime>(
    ...
    fn visit<R: Runtime>(
        ...
        for item in items {
            // Separators carry no mutable property (visibility is structural).
            if matches!(item, TrayItem::Separator(_)) {
                continue;
            }
            match handles.get(item_id(item)) {
                Some(handle) => handle.update(item)?,
                // The native menu and the snapshot diverged. `same_structure`
                // gates this path, so it means a programming error, not input.
                None => {
                    return Err(format!("no native handle for tray item {}", item_id(item)));
                }
            }
...
/// Walk the snapshot for `id`, returning its label and action.
fn find_action_entry<'a>(
    snapshot: &'a TrayMenuSnapshot,
    id: &str,
) -> Option<(&'a str, &'a crate::tray_model::TrayAction)> {
    fn visit<'a>(
        items: &'a [TrayItem],
        id: &str,
    ) -> Option<(&'a str, &'a crate::tray_model::TrayAction)> {
        for item in items {
            match item {
                TrayItem::Action(value) if value.id == id => {
                    return Some((&value.label, &value.action))
                }
                TrayItem::Check(value) | TrayItem::Radio(value) if value.id == id => {
                    return Some((&value.label, &value.action))
                }
                TrayItem::Submenu(value) => {
                    if let Some(found) = visit(&value.items, id) {
                        return Some(found);
                    }
                }
                _ => {}
...
fn find_action<'a>(
    ...
    find_action_entry(snapshot, id).map(|(_, action)| action)
}
...
mod tests {
    ...
    #[test]
    fn group_label_is_part_of_the_structure() {
        // A labelled group renders as a native submenu, so a label change must
        // rebuild rather than patch (an item label stays a property change).
        let plain = snap(vec![group("g", 0, vec![core_action_item("a", 0)])]);
        let mut labelled = plain.clone();
        labelled.groups[0].label = Some("Section".into());
        assert!(!same_structure(&plain, &labelled));

        let mut relabelled_item = plain.clone();
        if let TrayItem::Action(v) = &mut relabelled_item.groups[0].items[0] {
            v.label = "renamed".into();
        }
        assert!(same_structure(&plain, &relabelled_item));
    }
    ...
    fn item_helpers_pin_kind_ids_orders_and_visibility() {
        ...
        assert!(items.iter().all(item_visible));
    }
}
