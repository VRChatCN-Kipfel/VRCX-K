...
**如用户无特殊说明，一律写成：`Type(Scope): 中文描述`。**

```
feat(host): 接入上游 plugin-timer，去掉手写定时器
fix(ui): 挂载期拿到的生命周期响应不再覆盖 ready 事件
docs(m2): 钉住前端扩展协议，并更正我自己两处判断
```

- **Type 用既有词汇，不新造**（对照 `git log` 的实际取值）：`feat` / `fix` / `docs` / `refactor` / `test` / `chore` / `ci`。
- **Scope（同一批词）**：`host` / `ui` / `shell`（= `src-tauri`）/ `build` / `docs` / `probes` / `m2`（以及后续 `f1`… 里程碑）。**可选**——改动跨面时用 `feat(host,ui): …` 或省掉括号。
- **描述用中文**，写"改了什么/为什么"，不写"修改了文件"。
- 冒号后用**一个空格**；不要在头部堆无关信息。
- ⚠ **这是对历史惯例的显式转向**：本文件生效前仓库内的提交头部**绝大多数是英文**（`docs(host): failure isolation applies to every plugin`）。那批**不回改**；新提交按本结构写。
- **仅当用户明确要求时**才改用英文（或其他）描述——用户的当场措辞**优先于本条默认**。

### 提交正文（默认结构）

正文**默认也写中文**，三段（可按改动大小增删，但不写空话）：

1. **改了什么**
2. **为什么**（含被否掉的替代方案与理由）
3. **怎么验证的**（命令 + 结果；未验证的部分**如实标注**）

> 本仓库既有的长正文（例如 `beacf5a5`）在结构上就是这三段，只是语言是英文——**结构照旧，语言改成中文**。

### 其余

- **条件 `commit.gpgsign=true`**：在有条件的情况下尽量对所有提交进行GPG签名，如果当前环境不存在GPG签名应当先询问并指引签名方式（签名错误例外）。
- **保留既有标记**：不跑 CI 的提交照旧带 `[ci skip]`；引用 issue 照旧带 `(#NN)`——两者放在**头部描述的末尾，不占正文**（`git log --oneline` 的既有效果）。
- **不写 `Co-Authored-By` 之类的署名 trailer**，除非用户当轮明确要求。
