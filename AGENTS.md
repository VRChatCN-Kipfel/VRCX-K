...
| **手文件能力的端到端验证（真 kkrpc + 真管道 + 真生产源码）** | [`docs/probes/hands-e2e/`](docs/probes/hands-e2e/run.mjs) —— 用 `#[path]` 直接编译 `hands.rs` / `kkrpc_peer.rs`（**是生产源码，不是副本**），由真 `StreamingRPCChannel` 驱动，9/9 通过、4 MiB 双向字节一致（~112 MiB/s）。跑法：`cargo build --release --locked --manifest-path src-tauri/Cargo.toml --example hands-e2e && node docs/probes/hands-e2e/run.mjs`。⚠ 不跑整个 Tauri app：它要 webview，无法在 CI 无头运行，而文件能力**完全不依赖 Tauri**。同目录 `hol.mjs` 测队头阻塞（实测 **p50 仅 1.16–1.28x、p95 51–59x**），**`hol-credit.mjs` 分离成因**：真因是 **`pump_producer` 在 reader 线程上跑完整批 credit 才回到 `read_line`**（不是「排队」也不是「对端忙」）。判别法 = 同一 credit 下批**中间** vs 批**末尾**探针：MID p95 随 credit **线性**（~1.7ms/未完成块），END 恒定≈空闲。它还**解释了那个 51x**：kkrpc 消费者首发 pull=32、每 16 块补一次 ⇒ 1.7×32≈54ms（实测 max 55.7）、1.7×16≈27ms（实测 p95 24.3）。⚠ **载体（raw）不是这个问题的解** —— 它只缩短每次迭代，不消除等待整批；⚠ **减小 credit 治标且买掉吞吐**（credit=1 时 87 vs 171 MiB/s），且**每 pull 只发一块会死锁**（kkrpc 的消费者 `!(consumedSincePull < 16)` 才补货） |
