...
| PV-5 | copy-ready **候选**清单（cp 判定准则：①路径内全部提交作者 ∈ org 候选 {XChen446, RainyN0077, 1zyao, mobaiQWQ}；②无上游 fork 作者提交；③无 FuLu 系 "Merge upstream/master"；④`--follow` 溯源链最老节点也是 org 作者）：adapter/ 三引擎 + adapter/index.js + 全部 adapter 测试 + migrations/ 全族（index.js/migrations.js/_template.map/16 两份 .map/全部测试）+ pullEngine/pushEngine 及测试 + feed.test —— **≈30+ 个文件**（t5 与 cap-3 数字口径见 PV-4）<br>⚠ **2026-09 复核修正**：原列 `configRepository.js` —— **应剔除**，它是 PV-6 的例外之一（`--follow` 最老为 pypy）。清单与例外表当时自相矛盾。复核后 **32 个文件逐条满足四条准则**（`configRepository.test.js` 保留，它本身干净）。 | t5 + **captain 复核** | **候选非结论**（可用性判断归 captain/用户） |
| PV-5a | ⚠ **准则 ④ 的盲区（2026-09 复核新增）**：`--follow` 只跟 **rename 链**，**对 inline 合流是盲的** —— `adapter/SQLiteAdapter.js` 的 `--follow` 实测「全部 org」，但 PV-6 指出它 inline 自曾含上游作者写的 `sqlite.js`。⇒ **"④ 干净"只能证明"没有 rename 继承"，不能证明"内容无上游血统"**。与 `08` 那 12 个 rename/copy 文件是**同一类缺口**（见 `08-repo-wide-provenance.md` 修正）。取用前若需强证据，应补内容级对拍（blame / 与上游同名文件的 diff）。 | **captain 复核** | 事实（方法学边界） |
