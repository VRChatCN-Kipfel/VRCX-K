...
> 那是 SDK 的事）；但提案可以决定的那一半 —— **四个原语、流式背压、取消、错误码** ——
> 已在 `src-tauri/src/hands.rs` 落地，并由
> [`probes/hands-e2e/`](probes/hands-e2e/run.mjs) 对**真 kkrpc 2.1.0** 端到端验证
> （9/9，实测 4 MiB 文件 **~112 MiB/s**，双向字节一致）。
...
>
> ⚠ **本次落地**：`hands.stat` / `hands.read` / `hands.write` / `hands.watch` 已在 Rust 侧
> 实现并可跑；**它们不构成 SDK**，也**不主张排期**（§9 第 7 项仍未决）。SDK 侧要做的
> 是把它们包成 `ctx.hands`（含审计插针，§6）。
...
- **跨机未测**。本地管道是**下界**；异机上一块的写入时间随链路放大，尾部绝对值会更差。
  **本测量不能推出跨机结论**（transport-lab §8 同款纪律）。
...
**① kkrpc 的流式帧没有二进制载体 —— 形状由发送方决定，而不是协议决定。**
第一次端到端跑就撞上了：host 侧 `yield new Uint8Array(...)` 得到的**不是二进制**，
而是 `{"0":65,"1":66,…}`（11.4x 膨胀），Rust 侧报 `unrecognised stream value shape`。
⇒ **base64 不是「可选优化」，是流式通道上的实际契约**：`hands.write` 的调用方
**必须**自己编码。手侧现在三种形状都认（base64 / Node `Buffer` / `Uint8Array` 数字键），
因为「发送方忘了编码」若变成静默错读或不可解的错误，都比「正确但费带宽」更糟 ——
代价记录在案，留给上游修。见 `kkrpc_peer.rs::decode_chunk`。
