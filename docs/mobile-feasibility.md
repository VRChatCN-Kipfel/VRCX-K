...
> **宿主侧对应物见 [`host-sessions.md`](host-sessions.md)**：手机是第一个真正的第二客户端，但"会话/设备管理"的需求**不止于移动端**（Web 客户端、第二窗口同理）。该文档记录宿主侧缺什么、集成在哪、以及一条关键推论——**持久凭据必须独立于每次启动的 token**（端口随机 + token 每次重新生成）。
