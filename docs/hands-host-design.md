...
所以延迟可能部分来自**对端忙**而非**排队**。两者修法不同，本测量**不足以区分**，
已如实记录，不做因果断言。

**已确认的一条具体机制**（代码位置，非推测）：`base64` 解码跑在**reader 线程**上 ——
`kkrpc_peer.rs` 的读循环直接调 `dispatch`（`:382`），而 `dispatch_stream_data` 在其中
同步做 `decode_chunk`（`:831`）。**而应答交互调用的也是这个线程。**
⇒ 「对端忙」不是猜测，`decode_chunk` 已经是一个具体嫌疑点，且它**换 raw 就会消失**
（raw 不需要解码）。这与调研建议的「先分离成因」直接对上：**若成因是对端忙，
修法是换载体，不是加第二条通道。**
...
2. **§5 的取舍 —— 三个可独立决定的问题**：
   - **(i) 载体是否换 raw 二进制？** 单管道上已验证可行（1.8–2.1x），代价是**两端都改**
     （JS platform+codec **加上** `kkrpc_peer.rs` 的行读取循环）。**不依赖**通道数。
   - **(ii) 是否加第二条通道？** 成本是 socket + 握手 + 重连/保活 + `host.rs` 的 ownership。
     判据需要**跨机尾部**数据（本机 loopback 是下界）。
   - **(iii) 尾部成因是排队还是对端忙？** `hol.mjs` 已具备分离所需插桩；
     **两者修法不同**，未分离前不该为任何一侧投入。
