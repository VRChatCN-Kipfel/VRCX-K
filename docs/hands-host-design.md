...
> ⚠ **上表与下面两条结论都写在修复之前，保留作历史记录。** 真因后来被
> `hol-credit.mjs` 分离出来（**不是**这里的「逐帧持锁」推测），并且已经修掉 ——
> 见下方「✅ 已修：生产者改为自己的线程」。修后 **p95 70x → ~9x**，但
> **p50 由 1.03x 退到 ~8x**（就是这里说的那把写锁，现在成了主要成本）。

**这对 SDK 的含义变了**：

- 提案 §5 的三条出路里，**(c) 不解决**比原来更站得住：p50 无感的卡顿，
  用户多半感觉不到「托盘卡住」；
- ~~但 **p95 51x 是真实的**~~ → **已修**（见下），且**本地管道是下界** —— 异机
  （提案 §7 的场景）上一块的写入时间随链路放大。**跨机仍未测**。
- ⚠ **本测量不能推出跨机结论**（transport-lab §8 同款纪律）：loopback 测不出慢链路。

⚠ **成因已分离（2026-09，实测），并已修复。** 此前这里写「可能是排队，也可能是对端忙」——
...
##### ✅ 已修：生产者改为自己的线程（2026-09）

`pump_producer` 已删除，换成 `Peer::spawn_producer` —— 照
[`Peer::open_event_stream`] 的形（那处早就为 watch 解决过同一个危险），
生产者在**自己的线程**上等 credit、发块；`pull` 只做
`ProducerStream::add_credit`（`Condvar` 唤醒，**不是轮询**）。

**实测（同一探针、同一载荷）：**

| | 修前 | 修后 |
|---|---|---|
| `hol.mjs` p95 | **70.25x** | **~8–10x** |
| `hol.mjs` p50 | 1.03x | **~7–9x**（回退） |
| 吞吐 | 156 MiB/s | ~183 MiB/s |
| `hol-credit.mjs` | MID **随 credit 线性**（22/37/74/196x） | **不再随 credit 缩放**（H3 PARTIAL） |

⚠ **尾部修好了，中位数反而退了 —— 两件事都要说。** 机制在 MID/END 两臂里：
p50 的代价**只在生产者正在写时出现**（MID ~1.0 ms），credit 用尽后消失
（END ~0.22 ms ≈ 空闲）。256 KiB 一块写一次约 1.4 ms，读者要拿同一把写锁就得等约一半
—— 实测 ~0.9 ms。**这是「独立写者共享同一条通道」固有的写锁争用，不是轮询没调好**：
把轮询 1 ms 改 50 µs、以及每块加一次 `sched_yield`，**都没能撼动它**。

⚠ **中位数这条在本设计内修不掉**：一条通道意味着生产者必须拿读者也要的同一把写锁。
要去掉它得加第二条载体（`docs/hands-prior-art.md` §2 调研过），那是**协议决定，不是调参**。
