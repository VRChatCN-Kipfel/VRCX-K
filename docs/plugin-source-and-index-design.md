...
> **⚠ 转正说明（待办）**：上述"旧工程"结论来自对 `origin/old/main` 分支的系统勘察，产出目前**只在 `.temp/legacy-recon/`**（该目录随时可删）。
> **这不是唯一副本该待的地方** —— 勘察结论转正为 `docs/legacy-database-recon.md` 之前，本节对旧工程的引用**只保留结论，不依赖 `.temp/` 路径**。
