...
**已拍板语义（用户决定）：所有插件同等对待——base 包及其子包/插件与外部插件一样，失败一律隔离。**

> 理由：否则将来会出现「**我们的插件与某个外部插件冲突导致崩溃，却没有排查手段**」。若给 base 插件"致命"特权，一次冲突就会表现为**整个应用打不开且归因不明**——正是要避免的场景。

**规避方向（供 M2-4 / #18 采纳）**：

1. **拒绝/失败要发生在 entry 创建之前**（读 manifest → 校验 → 不过则不 `create`），从源头上不产生 FAILED fiber；
2. `waitForIncludeReady` **去掉「任一 FAILED 即致命」**，改为「记录并继续」，使单个插件的失败**不阻断宿主就绪**（**不区分 base / 用户插件**）；
3. 补 **`unhandledRejection` / `uncaughtException` 兜底**（当前完全缺失），把进程级逃逸降为日志；
4. **失败必须可归因**——"没有排查手段"的直接解药：日志/UI 须能回答「**哪个插件没起来、为什么**」（`entry.id` 后缀 + 错误摘要）。

> 注：1 与 2 是**两道不同的闸门**——1 拦"格式不合法"，2 兜"代码自己炸"。manifest 校验再严也防不住后者。
...
| **插件失败隔离（§1.15）** | **所有插件同等对待**（含 base 及其子包/插件）：失败一律隔离、不阻断宿主就绪、且**必须可归因**。`waitForIncludeReady` 现有"任一 FAILED 即致命"须改 |
