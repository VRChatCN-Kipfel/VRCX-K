...
export const HOST_LIFECYCLE_SCHEMA_ID =
...
export function isHostSnapshot(value: unknown): value is HostSnapshot {
  ...
  if (
    ...
    return false
  return (
    candidate.schemaVersion === HOST_LIFECYCLE_SCHEMA_VERSION &&
    (candidate.phase === "stopped" ||
      candidate.phase === "starting" ||
      candidate.phase === "ready" ||
      candidate.phase === "stopping" ||
      candidate.phase === "backoff" ||
      candidate.phase === "failed") &&
    (candidate.desired === "running" ||
      candidate.desired === "stopped" ||
      candidate.desired === "app_exit") &&
    Number.isSafeInteger(candidate.generation) &&
    (candidate.generation ?? -1) >= 0 &&
    (candidate.generation ?? MAX_SAFE_INTEGER + 1) <= MAX_SAFE_INTEGER &&
    typeof candidate.phase === "string" &&
    typeof candidate.desired === "string" &&
    (candidate.pid === null ||
      (Number.isSafeInteger(candidate.pid) && (candidate.pid ?? -1) >= 0)) &&
    (candidate.port === null ||
      (Number.isSafeInteger(candidate.port) &&
        (candidate.port ?? -1) >= 0 &&
        (candidate.port ?? 65536) <= 65535)) &&
    Number.isSafeInteger(candidate.attempt) &&
    (candidate.attempt ?? -1) >= 0 &&
    (candidate.nextRetryMs === null ||
      (Number.isSafeInteger(candidate.nextRetryMs) && (candidate.nextRetryMs ?? -1) >= 0)) &&
...
function isHostExitSummary(value: unknown): value is HostExitSummary {
  ...
  return (
    ...
    (candidate.kind === "restart_requested" ||
      candidate.kind === "stopped" ||
      candidate.kind === "crashed")
  )
}
