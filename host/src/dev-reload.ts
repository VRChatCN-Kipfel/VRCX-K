...
export type ReloadStatus =
  ...
  | "timeout"           // reload exceeded RELOAD_TIMEOUT_MS
...
export function collectCacheKeysUnderRoots(
  roots: string[],
  cache: Record<string, unknown> = require.cache as unknown as Record<string, unknown>,
): string[] {
...
export function invalidateCache(roots: string[], cache: Record<string, unknown> = require.cache as unknown as Record<string, unknown>): number {
...
export async function reloadPluginEntry(entry: Entry, roots: string[], deps: ReloadDeps): Promise<ReloadResult> {
  ...
  const entryFile = entry.options.name.startsWith(".") && baseUrl
    ...
    : entry.options.name
  const invalidationSet = new Set<string>(collectCacheKeysUnderRoots(roots))
  ...
  for (const key of invalidationSet) {
    const existing = (require.cache as Record<string, unknown>)[key]
    if (existing !== undefined) cacheSnapshot.set(key, existing)
    delete (require.cache as Record<string, unknown>)[key]
  }
  ...
  try {
    ...
    for (const [key, value] of cacheSnapshot) {
      ;(require.cache as Record<string, unknown>)[key] = value
    }
  ...
  if (disposeError) {
    ...
    // over it — surface restart-required.
    return { status: "restart-required", entryId, phase: "swap", error: disposeError, durationMs: performance.now() - started }
  }
  ...
  try {
    ...
    return restored
      ? { status: "restored-old", entryId, phase: "swap", error, durationMs: performance.now() - started }
      : { status: "restart-required", entryId, phase: "swap", error, durationMs: performance.now() - started }
  }
...
async function withTimeout<T>(promise: Promise<T>, ms: number, label: string): Promise<T> {
  ...
  const timeout = new Promise<never>((_, reject) => {
    timer = setTimeout(() => reject(new Error(`reload ${label} timed out after ${ms}ms`)), ms)
    timer.unref?.()
  })
  try {
