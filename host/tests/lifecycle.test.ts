...
function _fakeCtx(signal: ShutdownSignal) {
  return {
    signal,
    registry: new Map(),
    fiber: {
...
test("extend pushes the deadline and returns remaining time", async () => {
  ...
  // deadline moved forward. Checked, not asserted: `begin()` guarantees a
  // deadline, but the accessor admits null, and a silent `undefined` here would
  // make the comparison below vacuously pass.
  const deadline = signal.deadline
  if (deadline === null) throw new Error("begin() did not set a deadline")
...
test("extend respects the hard cap", async () => {
  ...
  if (hardCap === null) throw new Error("begin() did not set a hard cap")
  expect(hardCap - Date.now()).toBeLessThanOrEqual(500 + 50)
  // deadline cannot exceed the hard cap
  const deadline = signal.deadline
  if (deadline === null) throw new Error("begin() did not set a deadline")
  expect(deadline).toBeLessThanOrEqual(hardCap)
})
